...
(defn find-rep [seen sum unseen]
  (if (seq unseen)
    (let [new-sum (+ sum (first unseen))
          new-unseen (rest unseen)]
      (if (contains? seen new-sum)
        new-sum
        (recur (conj seen new-sum) new-sum (rest unseen))))
    nil))
